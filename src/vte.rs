...
const GRID_LINE_COLOR: Color = Color { r: 0.2, g: 0.0, b: 0.0 }; // Dark gray; adjust RGB (0.0-1.0) as needed
...
impl Grid {
    pub fn new(cols: usize, rows: usize) -> Self {
        let default_fg = Color { r: 1.0, g: 1.0, b: 1.0 }; // White text
        let default_bg = Color { r: 0.0, g: 0.0, b: 0.0 }; // Black background
    ...
    pub fn clear(&mut self) {
        ...
        for r in &mut self.cells { r.fill(default_cell); }
...
impl AnsiGrid for Grid {
    ...
    fn clear_screen(&mut self) { self.clear(); }
    ...
    fn reset_attrs(&mut self) {
        ...
        self.bold = false; self.italic = false; self.underline = false; self.dim = false;
    }
    ...
    // `set_title` is provided as a default no-op on the trait; implement if you want title updates.
}
...
impl VteTerminal {
    pub fn new() -> Self {
        ...
        let mut cmd = CommandBuilder::new("bash"); // Changed to bash for simplicity
        ...
        area.set_draw_func(move |area, cr, _w, _h| {
            ...
            let rows = (area.height() as f64 / char_h_draw).max(1.0) as usize;
            {
                ...
                if g.cols != cols || g.rows != rows {
                    ...
                    // Resize PTY
                    if let Ok(mut m) = master_draw.lock() {
            ...
            // TEST TEXT - hardcoded to verify rendering
            cr.set_source_rgb(1.0, 0.0, 0.0); // Red for visibility on black
            cr.select_font_face("Monospace", FontSlant::Normal, FontWeight::Normal);
            cr.move_to(10.0, 20.0);
            cr.show_text("TEST RENDERING WORKS").unwrap();
            ...
            for r in 0..g.rows.min(rows) {
                for c in 0..g.cols.min(cols) {
                    ...
                    // Draw text if present (now draws ' ' too for full coverage, but skip '\0')
                    if cell.ch != '\0' {
                        cr.set_source_rgb(cell.fg.r, cell.fg.g, cell.fg.b);
                        cr.select_font_face("Monospace", FontSlant::Normal,
            ...
            cr.set_source_rgba(1.0, 1.0, 1.0, 0.6); // Semi-transparent white
        ...
        // Initial queue draw
        std::thread::sleep(std::time::Duration::from_millis(100)); // Brief pause for thread start
        area.queue_draw();
