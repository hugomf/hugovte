...
mod tests {
    ...
    impl AnsiGrid for MockGrid {
        ...
        fn use_alternate_screen(&mut self, enable: bool) {
            ...
            self.output.push_str(if enable {
                "[ALT_SCREEN_ON]"
            } else {
                "[ALT_SCREEN_OFF]"
            });
        ...
        fn set_application_cursor_keys(&mut self, _enable: bool) {
            self.output.push_str(&format!("[APP_CURSOR_KEYS_{}]",
                if _enable { "ON" } else { "OFF" }));
        }

        fn set_mouse_reporting_mode(&mut self, mode: u16, enable: bool) {
            self.output.push_str(&format!("[MOUSE_MODE_{}_{}]",
                mode, if enable { "ON" } else { "OFF" }));
        }

        fn set_focus_reporting(&mut self, _enable: bool) {
            self.output.push_str(&format!("[FOCUS_REPORTING_{}]",
                if _enable { "ON" } else { "OFF" }));
        }
    }
    ...
    fn legacy_byte_api_still_works() {
        ...
        let mut g = MockGrid::default();
        for &b in b"Hello\n" {
            // Note: process() was the old API, but we keep it for compatibility
            let mut temp_parser = AnsiParser::new();
            temp_parser.feed_bytes(&[b], &mut g);
        }
    ...
    fn cursor_nested_save_restore() {
        let mut p = AnsiParser::new();
        let mut let mut g = MockGrid::new();
        // Pos1: (3,5)
